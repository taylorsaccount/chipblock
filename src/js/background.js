  ...
  */
  ...
  updateIcon: function (tab_id, tab_url) {
    ...
    if (!utils.isRestrictedUrl(tab_url) &&
      ...
      iconFilename = {
        16: chrome.runtime.getURL("icons/icon.png"),
        32: chrome.runtime.getURL("icons/icon.png"),
        64: chrome.runtime.getURL("icons/icon.png")
      };
    } else {
      iconFilename = {
        16: chrome.runtime.getURL("icons/icon-disabled.png"),
        32: chrome.runtime.getURL("icons/icon-disabled.png"),
        64: chrome.runtime.getURL("icons/icon-disabled.png")
      };
    }
...
function connectWS() {
  ...
  ws.onopen = () => {
    ws.send(JSON.stringify({ type: "identify", client: "extension" }));
};
}
...
let socialRemaining = 5 * 60;

chrome.runtime.onMessage.addListener((message) => {
  if (message.type === "onSocialSite") {
    if (!socialTimer) {
      socialTimer = setInterval(() => {
        socialRemaining--;
        if (socialRemaining <= 0) {
          clearInterval(socialTimer);
          socialTimer = null;
          socialRemaining = 5 * 60;
          if (ws.readyState === WebSocket.OPEN) {
            ws.send(JSON.stringify({ type: "socialTimerExpired" }));
          }
        }
      }, 1000);
    }
...
chrome.runtime.onMessage.addListener(function(message, sender) {
    if (message.type === "social_site_detected") {
        const now = Date.now();
        if (ws && ws.readyState === WebSocket.OPEN && now - lastNewsSent > 60000) {
            lastNewsSent = now;
            ws.send(JSON.stringify({ type: "news_site", url: message.url }));
        }
    }
});
...
async function pushTrackerData(tabId) {
  ...
  const trackers = badger.tabData.getTrackers(tabId);

  ws.send(JSON.stringify({
    type: "tracker_update",
    url: hostname,
    trackers: Object.entries(trackers),
    fingerprinters: Object.entries(trackers)
      .filter(([, action]) => action === constants.BLOCK || action === constants.USER_BLOCK)
      .map(([domain]) => domain)
  }));
}
